  ...
  context '# Test module on supported platforms' do
    SUPPORTED.each do |f|
      ...
      context "on #{f['os']['family']} #{f['os']['release']['major']} with defaults" do
        ...
        it { is_expected.to contain_archive('/opt/wildfly10/modules/system/layers/base/org/postgresql/main/postgresql-42.1.4.jar') }
        it { is_expected.to contain_file('/opt/wildfly10/modules/system/layers/base/org/postgresql/main/postgresql-42.1.4.jar').that_subscribes_to('Archive[/opt/wildfly10/modules/system/layers/base/org/postgresql/main/postgresql-42.1.4.jar]') }
      end
    end
  end
end
